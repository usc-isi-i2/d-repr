...
impl<'a0: 'a, 'a> AlignsIter<'a0, 'a> {
  ///
  pub fn new(readers: &'a [&'a0 Box<dyn RAReader + 'a0>], align_funcs: &'a mut [AlignmentFunc<'a0>], ys: &'a mut [Box<Vec<Index>>], source: &[Index], source_val: &Value, target: &'a mut [Index]) -> AlignsIter<'a0, 'a> {
    ...
    match unsafe { &mut (*(&mut align_funcs[0] as *mut AlignmentFunc)) } {
      ...
      AlignmentFunc::Multiple(mfunc) => {
        ...
        sub_iters.push(mfunc.iter_alignments(source, source_val, unsafe { &mut (*(ys[0].as_mut() as *mut Vec<Index>)) }));
      }
    }
