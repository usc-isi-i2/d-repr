...
use engine::executors::{Executor, PhysicalOutput, PhysicalResource};
use engine::lang::Description;
use engine::writers::stream_writer::OutputFormat;
...
pub fn test_executors() {
  ...
  for dataset in datasets {
    let resources: Vec<PhysicalResource> = dataset.resources.into_iter().map(|rpath| PhysicalResource::File(rpath)).collect();
    let description: Description = serde_json::from_reader(File::open(dataset.model).unwrap()).expect("Invalid description file");

//    if dataset.dataset_dir.find("s03").is_none() {
//      continue;
//    }
    println!("dataset: {}", dataset.dataset_dir);
    
    for output in &dataset.outputs {
      let exc_output = Path::new(&dataset.dataset_dir).join(format!("tmp_output.{:?}", output.1).to_lowercase()).to_str().unwrap().to_string();
      (Executor {
  ...
  for e0 in fs::read_dir(&test_resource_dir)? {
    ...
    if dataset_dir.is_dir() {
      ...
      for e1 in fs::read_dir(&dataset_dir)? {
        ...
        if file.file_name().unwrap().to_str().unwrap().starts_with("output") {
      ...
      // get model
      let model = dataset_dir.as_path().join("model.json").to_str().unwrap().to_string();
      
      resources.sort();
      datasets.push(Dataset { model, resources, outputs, dataset_dir: dataset_dir.to_str().unwrap().to_string() });
    }
  }
