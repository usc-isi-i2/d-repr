...
def test_get_prop_as_ndarray(ds01: ArrayBackend, ds02: ArrayBackend, ds03: ArrayBackend):
    values = [
        # ds01
    ...
    for ds in [ds01, ds02, ds03]:
        for c in ds.sm.c("mint:Variable"):
            cls = ds.get_class_by_id(c.node_id)
            data = cls.get_data_prop_as_ndarray(
                c.p("rdf:value")[0].edge_id,
                [c.p("mint-geo:lat")[0].edge_id, c.p("mint-geo:long")[0].edge_id])

            assert np.allclose(data.data, np.asarray(values.pop(0)))
