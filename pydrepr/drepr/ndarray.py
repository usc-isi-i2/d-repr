...
class NDArrayGraph:
    ...
    def get_property_as_ndarray(self, class_id: str, predicate: str, index_preds: List[str]) -> Tuple[np.ndarray, Optional[List[str]]]:
        """
        ...
        n_records = np.prod(self.table_shps[class_id])
        ...
        if isinstance(column, ColArray):
            ...
            index_col_ids = set(index_preds)
            col_size = column.get_original_size()
            index_col_shp = [table[cid].get_original_size() for cid in index_preds]
            if all(idx in index_col_ids for idx in column.index_by[:len(index_col_ids)]) and len(
                    column.index_by) >= len(index_col_ids):
            ...
            new_array = np.tile(column.value, self.table_shps[class_id])
            # now reshape to the shape of index columns
            target_shp = [table[cid].get_original_size() for cid in index_preds]
    ...
    def get_property_as_array(self, class_id: str, predicate: str):
        """
