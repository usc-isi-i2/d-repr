...
if __name__ == '__main__':
    ...
    for dataset_dir in sorted(wdir.iterdir()):
        ...
        models = [file for file in dataset_dir.iterdir() if file.name.endswith("model.yml")]
        for model in models:
            ...
            try:
                execute(ds_model, resources, FileOutput(dataset_dir / f"{dsid}.model.out", OutputFormat.TTL))
            except SystemError:
                print("Error while executing the D-REPR engine for dataset", dataset_dir.name)
                pass
