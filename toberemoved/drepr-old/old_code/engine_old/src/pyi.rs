...
py_module_initializer!(drepr, initdrepr, PyInit_drepr, |py, m | {
  ...
  m.add(py, "add_resource", py_fn!(py, add_resource(ptr: usize, rid: String, rpath: String, rser: String)))?;
  m.add(py, "add_preprocess", py_fn!(py, add_preprocess(ptr: usize, pser: String)))?;
  m.add(py, "add_variable", py_fn!(py, add_variable(ptr: usize, vid: String, vser: String)))?;
  m.add(py, "add_alignment", py_fn!(py, add_alignment(ptr: usize, aser: String)))?;
  m.add(py, "add_sm", py_fn!(py, add_sm(ptr: usize, sm: String)))?;
  m.add(py, "exec2str", py_fn!(py, exec2str(ptr: usize, format: String)))?;
  m.add(py, "exec2turtle_file", py_fn!(py, exec2turtle_file(ptr: usize, fpath: String)))?;
  Ok(())
});
...
fn add_resource(py: Python, ptr: usize, rid: String, rpath: String, rser: String) -> PyResult<bool> {
