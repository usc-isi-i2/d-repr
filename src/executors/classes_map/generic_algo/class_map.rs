...
use crate::executors::classes_map::generic_algo::{
  generic_mandatory_dprop_map, generic_mandatory_oprop_map, generic_optional_buffered_oprop_map,
  generic_optional_dprop_map, generic_optional_oprop_map,
};
...
pub fn generic_class_map(
  ...
  let mut oprop_aligns = class_plan
    .object_props
    .iter()
    .map(|a| build_align_func(&readers, desc, a.get_alignments()))
    .collect::<Vec<_>>();
  let mut oprop_indices = class_plan
    .object_props
    .iter()
    .map(|a| {
      a.get_attr()
        .path
        .get_initial_step(readers[a.get_attr().resource_id].as_ref())
    })
    .collect::<Vec<_>>();

  let mut buffered_oprop_aligns = class_plan
    .buffered_object_props
    .iter()
    .map(|a| build_align_func(&readers, desc, a.get_alignments()))
    .collect::<Vec<_>>();
  let mut buffered_oprop_indices = class_plan
    .buffered_object_props
    .iter()
    .map(|a| {
      a.get_attr()
    ...
    || class_plan.object_props.iter().any(|op| !op.is_optional())
  {
    ...
    let mut buf_writer =
      BufferWriter::with_capacity(class_plan.data_props.len(), class_plan.object_props.len());
    ...
    loop {
      ...
      if is_new_subject {
        for (di, dplan) in class_plan.data_props.iter().enumerate() {
          should_keep_record = generic_mandatory_dprop_map(
            &readers,
            &mut buf_writer,
            dplan,
            &mut dprop_aligns[di],
      ...
      if should_keep_record {
        ...
        for (target_cls_id, pred_id, object, is_object_blank) in buf_writer.object_props.iter() {
          cls_writer.write_object_property(
            *target_cls_id,
            &subj_id,
            *pred_id,
            object,
        ...
        /// Write literal inverse properties of a class
        /// TODO: this hack only works for TurtleWriter as we leverage the specific implementation
        /// of begin_record of existing subj_id and write_object_property
        write_literal_inverse_properties(cls_writer, class_plan, &subj_id, is_subj_blank);
      }
    ...
    loop {
      ...
      for (oi, oplan) in class_plan.buffered_object_props.iter().enumerate() {
        generic_optional_buffered_oprop_map(
          ...
          cls_writer,
          oplan,
          &mut buffered_oprop_aligns[oi],
          &subj_id,
          subj_val,
          subj_iter.value(),
          &mut buffered_oprop_indices[oi],
          is_new_subject,
        );
      }
...
pub fn write_literal_inverse_properties(
  ...
  if class_plan.literal_inverse_props.len() == 0 {
    return;
  }
  // println!("{:?}", class_plan);
  let is_new_subj = cls_writer.begin_record(&subj_id, is_subj_blank);
  // assert!(!is_new_subj);
  for lplan in class_plan.literal_inverse_props.iter() {
    cls_writer.write_object_property(
      100000,
      lplan.value.as_str(),
      lplan.predicate_id,
