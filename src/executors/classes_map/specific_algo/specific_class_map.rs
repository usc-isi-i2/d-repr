use readers::prelude::{RAReader, Index};
...
use crate::execution_plans::classes_map_plan::object_prop::{BlankObject, ObjectProp, IDObject};
...
pub fn analyze_specific_algo_strategy(class_plan: &ClassMapPlan) -> Option<String> {
  ...
  let (
    Ob_Fs_Tt_oprops, Ob_Fs_Tf_oprops, Ob_Fm_Tt_oprops, Ob_Fm_Tf_oprops,
    Oi_Fs_Tt_Mt_oprops, Oi_Fs_Tf_Mt_oprops, Oi_Fm_Tt_Mt_oprops, Oi_Fm_Tf_Mt_oprops,
    Oi_Fs_Tt_Mf_oprops, Oi_Fs_Tf_Mf_oprops, Oi_Fm_Tt_Mf_oprops, Oi_Fm_Tf_Mf_oprops
  ) = split_oprops(class_plan);
  
  if Mt_dprops.len() > max_Mt_dprops ||  Mf_dprops.len() > max_Mf_dprops ||
    Ob_Fs_Tt_oprops.len() > max_Ob_Fs_Tt_oprops || Ob_Fs_Tf_oprops.len() > max_Ob_Fs_Tf_oprops || Ob_Fm_Tt_oprops.len() > max_Ob_Fm_Tt_oprops || Ob_Fm_Tf_oprops.len() > max_Ob_Fm_Tf_oprops ||
    Oi_Fs_Tt_Mt_oprops.len() > max_Oi_Fs_Tt_Mt_oprops || Oi_Fs_Tf_Mt_oprops.len() > max_Oi_Fs_Tf_Mt_oprops || Oi_Fm_Tt_Mt_oprops.len() > max_Oi_Fm_Tt_Mt_oprops || Oi_Fm_Tf_Mt_oprops.len() > max_Oi_Fm_Tf_Mt_oprops ||
    Oi_Fs_Tt_Mf_oprops.len() > max_Oi_Fs_Tt_Mf_oprops || Oi_Fs_Tf_Mf_oprops.len() > max_Oi_Fs_Tf_Mf_oprops || Oi_Fm_Tt_Mf_oprops.len() > max_Oi_Fm_Tt_Mf_oprops || Oi_Fm_Tf_Mf_oprops.len() > max_Oi_Fm_Tf_Mf_oprops
    {
    return None;
  }
...
pub fn specific_class_map(readers: &[Box<dyn RAReader>], cls_writer: &mut dyn StreamClassWriter, desc: &Description, class_plan: &ClassMapPlan) -> bool {
  ...
  let (mut Ob_Fs_Tt_oprop_aligns, mut Ob_Fs_Tt_oprop_indices) = get_Ob_Fs_align_and_index(readers, desc, &Ob_Fs_Tt_oprops);
  let (mut Ob_Fs_Tf_oprop_aligns, mut Ob_Fs_Tf_oprop_indices) = get_Ob_Fs_align_and_index(readers, desc, &Ob_Fs_Tf_oprops);
  let (mut Ob_Fm_Tt_oprop_aligns, mut Ob_Fm_Tt_oprop_indices) = get_Ob_Fm_align_and_index(readers, desc, &Ob_Fm_Tt_oprops);
  let (mut Ob_Fm_Tf_oprop_aligns, mut Ob_Fm_Tf_oprop_indices) = get_Ob_Fm_align_and_index(readers, desc, &Ob_Fm_Tf_oprops);
  
  let (mut Oi_Fs_Tt_Mt_oprop_aligns, mut Oi_Fs_Tt_Mt_oprop_indices) = get_Oi_Fs_align_and_index(readers, desc, &Oi_Fs_Tt_Mt_oprops);
  let (mut Oi_Fs_Tf_Mt_oprop_aligns, mut Oi_Fs_Tf_Mt_oprop_indices) = get_Oi_Fs_align_and_index(readers, desc, &Oi_Fs_Tf_Mt_oprops);
  let (mut Oi_Fm_Tt_Mt_oprop_aligns, mut Oi_Fm_Tt_Mt_oprop_indices) = get_Oi_Fm_align_and_index(readers, desc, &Oi_Fm_Tt_Mt_oprops);
  let (mut Oi_Fm_Tf_Mt_oprop_aligns, mut Oi_Fm_Tf_Mt_oprop_indices) = get_Oi_Fm_align_and_index(readers, desc, &Oi_Fm_Tf_Mt_oprops);
  let (mut Oi_Fs_Tt_Mf_oprop_aligns, mut Oi_Fs_Tt_Mf_oprop_indices) = get_Oi_Fs_align_and_index(readers, desc, &Oi_Fs_Tt_Mf_oprops);
  let (mut Oi_Fs_Tf_Mf_oprop_aligns, mut Oi_Fs_Tf_Mf_oprop_indices) = get_Oi_Fs_align_and_index(readers, desc, &Oi_Fs_Tf_Mf_oprops);
  let (mut Oi_Fm_Tt_Mf_oprop_aligns, mut Oi_Fm_Tt_Mf_oprop_indices) = get_Oi_Fm_align_and_index(readers, desc, &Oi_Fm_Tt_Mf_oprops);
  let (mut Oi_Fm_Tf_Mf_oprop_aligns, mut Oi_Fm_Tf_Mf_oprop_indices) = get_Oi_Fm_align_and_index(readers, desc, &Oi_Fm_Tf_Mf_oprops);
...
fn split_dprops<'a0: 'a, 'a>(class_plan: &'a ClassMapPlan<'a0>) -> (Vec<&'a DataProp<'a0>>, Vec<&'a DataProp<'a0>>) {
...
fn split_oprops<'a0: 'a, 'a>(class_plan: &'a ClassMapPlan<'a0>) -> (
  Vec<&'a BlankObject<'a0>>, Vec<&'a BlankObject<'a0>>, Vec<&'a BlankObject<'a0>>, Vec<&'a BlankObject<'a0>>,
  Vec<&'a IDObject<'a0>>, Vec<&'a IDObject<'a0>>, Vec<&'a IDObject<'a0>>, Vec<&'a IDObject<'a0>>,
  Vec<&'a IDObject<'a0>>, Vec<&'a IDObject<'a0>>, Vec<&'a IDObject<'a0>>, Vec<&'a IDObject<'a0>>,
) {
...
#[inline]
fn get_dalign_and_index<'a>(readers: &'a [Box<dyn RAReader + 'a>], desc: &Description, dprops: &Vec<&DataProp>) -> (Vec<Box<dyn SAlignmentFunc + 'a>>, Vec<Vec<Index>>) {
  let dprop_aligns = dprops.iter()
    .map(|a| build_align_func(&readers, desc, &a.alignments).into_single())
    .collect::<Vec<_>>();
  let dprop_indices = dprops.iter()
...
#[inline]
fn get_Ob_Fs_align_and_index<'a>(readers: &'a [Box<dyn RAReader + 'a>], desc: &Description, oprops: &Vec<&BlankObject>) -> (Vec<Box<dyn SAlignmentFunc + 'a>>, Vec<Vec<Index>>) {
  let oprop_aligns = oprops.iter()
    .map(|a| build_align_func(&readers, desc, &a.alignments).into_single())
    .collect::<Vec<_>>();
  let oprop_indices = oprops.iter()
    .map(|p| p.attribute.path.get_initial_step(readers[p.attribute.resource_id].as_ref()))
    .collect::<Vec<_>>();
...
#[inline]
fn get_Ob_Fm_align_and_index<'a>(readers: &'a [Box<dyn RAReader + 'a>], desc: &Description, oprops: &Vec<&BlankObject>) -> (Vec<Box<dyn MAlignmentFunc + 'a>>, Vec<Vec<Index>>) {
  let oprop_aligns = oprops.iter()
...
#[inline]
fn get_Oi_Fs_align_and_index<'a>(readers: &'a [Box<dyn RAReader + 'a>], desc: &Description, oprops: &Vec<&IDObject>) -> (Vec<Box<dyn SAlignmentFunc + 'a>>, Vec<Vec<Index>>) {
  let oprop_aligns = oprops.iter()
    .map(|a| build_align_func(&readers, desc, &a.alignments).into_single())
    .collect::<Vec<_>>();
  let oprop_indices = oprops.iter()
...
#[inline]
fn get_Oi_Fm_align_and_index<'a>(readers: &'a [Box<dyn RAReader + 'a>], desc: &Description, oprops: &Vec<&IDObject>) -> (Vec<Box<dyn MAlignmentFunc + 'a>>, Vec<Vec<Index>>) {
  let oprop_aligns = oprops.iter()
    .map(|a| build_align_func(&readers, desc, &a.alignments).into_multiple())
    .collect::<Vec<_>>();
  let oprop_indices = oprops.iter()
