...
use crate::execution_plans::classes_map_plan::subject::{
  BlankSubject, ExternalIDSubject, InternalIDSubject, Subject,
};
use crate::execution_plans::pseudo_id::ClassPseudoID;
use crate::lang::{Description, GraphNode, DREPR_URI};
...
pub struct ClassMapPlan<'a> {
  ...
  pub literal_inverse_props: Vec<LiteralProp>,
...
impl<'a> ClassMapPlan<'a> {
  pub fn new(
    ...
    class_id: usize,
    class2subj: &[usize],
    inference: &AlignmentInference,
    edges_optional: &[bool],
    removed_edges: &[bool],
  ) -> ClassMapPlan<'a> {
    let subj = class2subj[class_id];
    let uri_dnode = match desc.semantic_model.outgoing_edges[class_id]
      .iter()
      .find(|eid| desc.semantic_model.edges[**eid].rel_label == DREPR_URI)
    {
      None => None,
      Some(eid) => {
        Some(desc.semantic_model.nodes[desc.semantic_model.edges[*eid].target].as_data_node())
      }
    };

    // generate other properties
    let mut literal_props = vec![];
    let mut literal_inverse_props = vec![];
    let mut data_props = vec![];
    let mut object_props = vec![];
    let mut buffered_object_props = vec![];

    for &eid in &desc.semantic_model.incoming_edges[class_id] {
      if let GraphNode::LiteralNode(n) = desc.semantic_model.get_source(eid) {
        literal_inverse_props.push(LiteralProp {
          predicate_id: eid,
          value: n.val.clone(),
        });
      }
        ...
        GraphNode::ClassNode(n) => {
          ...
          let prop = if n.is_blank_node(&desc.semantic_model) {
            ObjectProp::BlankObject(BlankObject {
              ...
              alignments,
              pseudo_id: ClassPseudoID::new(
                format!("_:{}", n.get_pseudo_prefix()),
                attribute.path.get_nary_steps(),
              ),
              predicate_id,
              class_id,
    ...
    let subj_pseudo_id = ClassPseudoID::new(
      format!(
        "_:{}",
        desc.semantic_model.nodes[class_id]
          .as_class_node()
          .get_pseudo_prefix()
      ),
    ...
    let subject = match uri_dnode {
      None => Subject::BlankSubject(BlankSubject {
        attr: subj_attr,
        pseudo_id: subj_pseudo_id,
      }),
      Some(uri_dnode) => {
        // get missing values from the real subjects
        let missing_values = desc.attributes[subj]
          .missing_values
          .iter()
          .map(|v| v.clone())
          .collect::<HashSet<_>>();
        ...
        if uri_dnode.attr_id == subj {
          Subject::InternalIDSubject(InternalIDSubject {
            attr: subj_attr,
            pseudo_id: subj_pseudo_id,
            is_optional: edges_optional[desc
              .semantic_model
              .get_edge(class_id, uri_dnode.node_id)
              .unwrap()
              .edge_id],
            missing_values,
          })
          ...
          Subject::ExternalIDSubject(ExternalIDSubject {
            ...
            pseudo_id: subj_pseudo_id,
            real_id: (
              &desc.attributes[uri_dnode.attr_id],
              inference.get_alignments(subj, uri_dnode.attr_id),
            ),
            is_optional: edges_optional[desc
              .semantic_model
              .get_edge(class_id, uri_dnode.attr_id)
              .unwrap()
              .edge_id],
            missing_values,
          })
  ...
  pub fn find_subject(
    ...
    if subjs.len() == 0 {
      if attrs.len() == 0 {
        ...
        // inference.infer_subject()
        let mut other_attrs = Vec::new();
        for &eid in &desc.semantic_model.outgoing_edges[class_id] {
          let target = desc.semantic_model.edges[eid].get_target(&desc.semantic_model);
          if target.is_class_node() {
            let target_subj = class2subj[target.get_node_id()];
            // we must have inferred the subject of the target class before (because of the topological sorting)
            assert!(target_subj < desc.attributes.len());
            other_attrs.push(target_subj);
          }
        }
        subjs = inference.infer_subject(&other_attrs);
      } else {
  ...
  pub fn is_optional(&self) -> bool {
    self.subject.is_optional()
      && self.data_props.iter().all(|p| p.is_optional)
      && self
        .object_props
        .iter()
        .chain(self.buffered_object_props.iter())
        .all(|p| p.is_optional())
  }
}
