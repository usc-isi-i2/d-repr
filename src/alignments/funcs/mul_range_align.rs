...
use readers::prelude::{Index, IndexIterator, RAReader, Value, UnknownRangeRefIter, KnownRangeRefIter};

use crate::alignments::MAlignmentFunc;

use super::sgl_range_align::{AlignedRangeStep, create_aligned_range_step_index, update_range_step};
use crate::lang::{RangeAlignment, Description};
...
impl<'a> MRangeAlignFunc<'a> {
  pub fn from_dim_align(ra_reader: &'a Box<dyn RAReader + 'a>, desc: &Description, dalign: &RangeAlignment) -> MRangeAlignFunc<'a> {
    ...
    let maligned_dims = target.path.get_nary_steps()
      .into_iter()
  ...
  pub fn new(ra_reader: &'a Box<dyn RAReader + 'a>, source_n_dims: usize, target_n_dims: usize, mut saligned_dims: Vec<AlignedRangeStep>, maligned_dims: Vec<MAlignedDimension>) -> MRangeAlignFunc<'a> {
    ...
    MRangeAlignFunc {
      ra_reader,
      saligned_dim_index: create_aligned_range_step_index(max(source_n_dims, target_n_dims), &saligned_dims),
      saligned_dims,
      maligned_dims,
      unfrozen_dims,
      unknown_upperbounds,
...
impl<'a0> MAlignmentFunc for MRangeAlignFunc<'a0> {
  fn iter_alignments<'a1: 'a, 'a>(&'a1 mut self, source: &[Index], _source_val: &Value, target: &'a mut [Index]) -> Box<dyn IndexIterator + 'a> {
    ...
    if self.has_unknown_dim {
      Box::new(UnknownRangeRefIter::new(
        self.ra_reader.as_ref(), target,
        &mut self.unfrozen_dims, &mut self.unknown_upperbounds,
        &mut self.lowerbounds, self.upperbounds.clone(),
        &mut self.neg_upperbounds, &mut self.steps))
    } else {
      Box::new(KnownRangeRefIter::new(target, &mut self.unfrozen_dims, &mut self.lowerbounds, &mut self.upperbounds, &mut self.steps))
    }
  }
}